...
export function renderCleanStatus(opts: PowerlineOptions): string {
  ...
  if (details.length > available) details = `${percent}% · $${opts.cost.toFixed(2)}`;
  const git = GitStatusCache.getStatus();
  const branch = git ? ` · ${git.branch}${git.isDirty ? '*' : ''}` : '';
  if (details.length + branch.length <= available) details += branch;
